...
const PermissionDescription: FC<{ permission: Permission }> = ({
	...
	switch (permission) {
		case Permission.base:
			return (
				<>
					Allow to read address, account balance, activity from
					unlocked wallet. Every time to connect YepWallet to dApp you
					have to put wallet password.
				</>
			);
		case Permission.locked:
			return (
				<>
					Allow to read address, account balance, activity from locked
					wallet.{" "}
					<InlineLink
						onClick={() =>
							ExtensionPlatform.openTab({
								url: "https://yepwallet.xyz/docs/permissions",
							})
						}
					>
						Read more <LinkIcon />
