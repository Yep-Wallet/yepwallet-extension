	...
	const onConfirm = async () => {
		...
		try {
			for (let i = 0; i < items.length; i++) {
				...
				if (i !== items.length - 1) {
					await askBackground<void>(timeout).message(
						"confirmSeqNo",
						seqno
					);
				}
			...
			setItems((s) =>
				s.map((item) =>
					item.isConfirmed ? item : { ...item, isSend: false }
				)
			);
		}
	};
	...
	if (!isConfirm) {
		return (
			<>
				<Column>
					<Text>
						The Ton Ledger App version 2.0 does not support sending
						multiple transfers per transaction.
					</Text>
					<Text>
						You may sign and send multiple transfers one by one.
					</Text>
					<Text>
						Please do not interrupt the execution of transactions to
						get multiple transfer expected result.
					</Text>
				</Column>
				<Gap />
