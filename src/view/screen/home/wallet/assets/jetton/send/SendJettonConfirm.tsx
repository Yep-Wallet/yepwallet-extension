...
export const SendJettonConfirm: FC<ConfirmProps> = ({
	...
	const transaction =
		state.transactionAmount != ""
			? parseFloat(state.transactionAmount)
			: 0.1;
	...
	return (
		<>
			...
			<Body>
				...
				<Quote>
					* The wallet sends an amount of TON to cover internal
					transaction costs. The rest of the TON that will not be used
					will be returned to the wallet.
				</Quote>

				{addressError && (
					<ErrorMessage>{addressError.message}</ErrorMessage>
				)}
				{jettonError && (
					<ErrorMessage>{jettonError.message}</ErrorMessage>
				)}
