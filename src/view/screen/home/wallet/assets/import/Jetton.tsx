...
export const ImportJetton = () => {
	...
	const [symbolError, setSymbolError] = useState<Error | undefined>(
		undefined
	);
	...
	const onSearch = useCallback(async () => {
		resetFullData();
		console.log("jettonFullDataAsync", minter);

		const { data, wallet, name } = await jettonFullDataAsync(minter);
		console.log("data, wallet, name ", data, wallet, name);
	...
	return (
		<>
			...
			<Body style={{ gap: 15 }}>
				...
				{errorMinter && (
					<ErrorMessage>{errorMinter.message}</ErrorMessage>
				)}
