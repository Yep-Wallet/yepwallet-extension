...
const decryptOutMessage = async (
	...
	if (outMessage.msg_data["@type"] === "msg.dataEncryptedText") {
		const yepwallet_decrypted_payload = decryptMessage(
			Buffer.from(outMessage.msg_data.text, "base64"),
			sharedKey
		);
		if (yepwallet_decrypted_payload) {
			track("standard");
		}
		...
		return {
			...
			msg_data: {
				...outMessage.msg_data,
				yepwallet_decrypted_payload,
			},
		};
	}
	if (outMessage.msg_data["@type"] === "msg.dataRaw") {
		const yepwallet_decrypted_payload = decryptMessageV1(
