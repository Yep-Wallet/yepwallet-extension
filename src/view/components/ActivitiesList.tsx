import { base64ToBytes } from "@yepwallet/web-sdk";
...
const CommentBlock: FC<{ msg: TonWebTransactionMessage }> = ({ msg }) => {
	const [hasComment, encrypted, value] = useMemo(() => {
		if (
			"yepwallet_decrypted_payload" in msg.msg_data &&
			msg.msg_data.yepwallet_decrypted_payload
		) {
			return [
				true,
				true,
				decodeText(msg.msg_data.yepwallet_decrypted_payload),
			] as const;
		}
