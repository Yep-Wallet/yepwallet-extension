...
const middleTopFaces = Array.from(
	...
	(_, i) => styled(TriangleMiddleTop)`
		animation-delay: -${2500 + i * 1000}ms;
		transform: rotateY(${22.5 + 45 * i}deg) translateY(-111px)
			translateZ(-31px) rotateX(-58deg) scaleX(0.13) scaleY(-0.36);
	`
);
...
const upFaces = Array.from(
	...
	(_, i) => styled(TriangleUp)`
		animation-delay: -${3500 + i * 1000}ms;
		transform: rotateY(${22.5 + 45 * i}deg) translateY(-22px)
			translateZ(0px) rotateX(-70deg) scaleX(0.13) scaleY(0.33);
	`
);
