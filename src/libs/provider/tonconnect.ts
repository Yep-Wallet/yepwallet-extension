...
import { YepWalletError } from "../entries/message";
import {
  TonConnectItemReply,
  TonConnectRequest,
...
export class TonConnect implements TonConnectBridge {
  ...
  walletInfo: WalletInfo = {
    name: "YepWallet",
    image:
      "https://github.com/Yep-Wallet/yepwallet-extension/blob/main/public/tonmask-logo.svg",
    about_url: "https://www.yepwallet.xyz/",
  };
  protocolVersion = 2;
    ...
    try {
      ...
      if (e instanceof TonConnectError) {
        ...
        return this.notify(
          formatConnectEventError(
            new TonConnectError(
              (e as YepWalletError).message ?? "Unknown error",
              mapErrorCode((e as YepWalletError).code)
            )
          )
        );
      }
